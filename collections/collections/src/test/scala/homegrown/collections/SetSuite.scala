...
class SetSuite extends FunSuite with Matchers {
  ...
  test("remove on a non empty Set should yield a new Set without the element") {
    val element = randomString
    val setWithElement: Set = Set.empty.add(element)

    setWithElement(element) shouldBe true

    val setWithoutElement: Set = setWithElement.remove(element)
  ...
  test("union on empty Set should yield an empty Set") {
    ...
    val empty = Set.empty
    val union = empty.union(empty)

    empty.isSubSetOf(union) shouldBe true
    empty.isSubSetOf(empty) shouldBe true

    Set.empty.union(Set.empty) shouldBe Set.empty
  }
  test("union on a non empty Set with an empty set should yield he original Set untouched") {
    val first = randomString
    val second = randomString
  ...
  test("union on two non empy Set should yield their union") {
    val a = randomString
    val b = randomString
    val c = randomString
    val d = randomString
